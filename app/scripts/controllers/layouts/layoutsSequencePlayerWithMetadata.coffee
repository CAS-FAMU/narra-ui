...
angular.module('narra.ui').controller 'LayoutsSequencePlayerWithMetadataCtrl', ($scope, $sce, $timeout, $routeParams, $window, $rootScope, $q, apiProject, apiVisualization, elzoidoPromises, uiGmapGoogleMapApi) ->
  # player
  $scope.player =
    api: undefined
    ready: false
  ...
  uiGmapGoogleMapApi.then ->
    ...
    $scope.$watch 'player.current', ->
      if !_.isUndefined($scope.sequence) && !_.isUndefined($scope.metadata)
        # get mark
